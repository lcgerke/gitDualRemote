...
func checkGitInstallation(out *ui.Output, results *DiagnosticResults) {
	...
	if err != nil {
		if !out.IsJSON() {
			out.Error(fmt.Sprintf("  ✗ Git not found: %v", err))
		}
		results.AddCheck("git_installation", "error", err.Error(), nil)
		return
	}
...
func checkVault(out *ui.Output, results *DiagnosticResults) *vault.Client {
	...
	if err != nil {
		if !out.IsJSON() {
			out.Error(fmt.Sprintf("  ✗ Vault client creation failed: %v", err))
		}
		results.AddCheck("vault_connectivity", "error", err.Error(), nil)
		return nil
	}
...
func checkStateFile(out *ui.Output, results *DiagnosticResults) *state.Manager {
	...
	if err != nil {
		if !out.IsJSON() {
			out.Error(fmt.Sprintf("  ✗ State manager failed: %v", err))
		}
		results.AddCheck("state_file", "error", err.Error(), nil)
		return nil
	}
	...
	if err != nil {
		if !out.IsJSON() {
			out.Error(fmt.Sprintf("  ✗ Failed to load state: %v", err))
		}
		results.AddCheck("state_file", "error", err.Error(), nil)
		return nil
	}
...
func runAutoFix(out *ui.Output, results *DiagnosticResults, stateMgr *state.Manager) {
	...
	if err != nil {
		if !out.IsJSON() {
			out.Error(fmt.Sprintf("  ✗ Failed to detect issues: %v", err))
		}
		results.AddCheck("auto_fix", "error", err.Error(), nil)
		return
	}
	...
	if err != nil {
		if !out.IsJSON() {
			out.Error(fmt.Sprintf("  ✗ Auto-fix failed: %v", err))
		}
		results.AddCheck("auto_fix", "error", err.Error(), nil)
		return
	}
